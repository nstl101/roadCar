...
vector<int> findWay(int st, int ed, int speed, map<int, Cross*>& crossMap, map<int, Road*>& roadMap) {
	...
	while (checkedCross.back() != ed) {
		for (int i = 1; i <= crossMap.size(); ++i) {
			if (crossChecked(i, checkedCross)) {
				continue;
			}
			for (int j = 0; j < checkedCross.size(); ++j) {
				if (searchMap[st][checkedCross[j]] < searchMap[st][i] - searchMap[checkedCross[j]][i]) {
...
int main(int argc, char* argv[])
{
	...
	for (auto& e : car_map)
	{
		...
		ans->pi = -1;
		ans->pos = 0;
		ans->path = findWay(car->st, car->dst, car->maxSpeed, cross_map, road_map);
		ans_map[car_id] = ans;
	}
	/*
